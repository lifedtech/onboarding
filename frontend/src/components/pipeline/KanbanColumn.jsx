...
export default function KanbanColumn({ phase, healthmates }) {
  ...
  return (
    <div className="flex flex-col w-72 shrink-0">
      {/* Column header */}
      <div className={`flex items-center justify-between mb-4 px-4 py-3 rounded-2xl bg-white/80 backdrop-blur-md shadow-sm border border-slate-200/50 border-b-[3px] ${config.accent}`}>
      ...
      >
        ...
        {healthmates.length === 0 && (
          <div className={`
            ...
            text-text-muted/60 text-xs font-bold transition-all duration-200 bg-white/40 backdrop-blur-sm
            ${isOver ? 'border-brand-teal text-brand-teal bg-white/80 shadow-sm' : 'border-slate-300/60'}
          `}>
